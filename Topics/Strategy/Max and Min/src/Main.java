...
class MaxFindingStrategy implements FindingStrategy {
    ...
    public int getResult(int[] numbers) {
        return numbers.length>0?Arrays.stream(numbers).max().getAsInt():Integer.MIN_VALUE;
    }
}
...
class MinFindingStrategy implements FindingStrategy {
    ...
    public int getResult(int[] numbers) {
        return numbers.length>0?Arrays.stream(numbers).min().getAsInt():Integer.MAX_VALUE;
    }
}
